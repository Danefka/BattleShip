...
AttackManager::AttackManager(Output<IGraphic> *output) {
    this->output = output;
...
void AttackManager::addRandomAbility() {
    ...
    switch (random) {
        case (0): {
            ...
            output->printMessage("\e[1;36madd a bombing attack in your bag \e[0m");
            return;
        }
        case (1): {
            DoubleAttack* doubleAttack = new DoubleAttack();
            abilities.push_back(doubleAttack);
            output->printMessage("\e[1;36madd a double attack in your bag \e[0m");
            return;
        }
        case (2): {
            ScannerAttack* scannerAttack = new ScannerAttack();
            abilities.push_back(scannerAttack);
            output->printMessage("\e[1;36madd a scanner attack in your bag \e[0m");
            return;
        }
        default: {
            return;
